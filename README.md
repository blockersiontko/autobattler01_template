...
- Konwencja nazewnictwa odbiega od standardu C# (`base_class`, `isAlive` pisane małą literą zamiast PascalCase, jak przyjęto dla publicznych klas/właściwości w .NET). Warto ujednolicić pod `Microsoft C# Coding Conventions` (np. `CharacterBase`, `IsAlive`).
