...
- **`archer` : base_class**, łucznik o zmiennej sile ataku. `Attack()` losuje wartość obrażeń z przedziału `Random.Range(5, 15)` i zadaje je celowi. `TakeDamage()` korzysta z domyślnej implementacji bazowej (brak redukcji obrażeń).
...
- Brak obsługi remisu/edge case'ów (np. jednoczesna śmierć obu postaci w tej samej klatce nie jest jednoznacznie obsłużona).
- `Random.Range(5, 15)` w wariancie na `int` ma górną granicę wyłączną (exclusive), więc realny zakres obrażeń łucznika to 5-14, a nie 5-15. Jeśli 15 miało być możliwym wynikiem, to jest to błąd.
- Konwencja nazewnictwa odbiega od standardu C# (`base_class`, `isAlive` pisane małą literą zamiast PascalCase, jak przyjęto dla publicznych klas/właściwości w .NET). Warto ujednolicić pod `Microsoft C# Coding Conventions` (np. `CharacterBase`, `IsAlive`) przed pokazaniem projektu rekruterowi.
- Dodawanie nowej jednostki wymaga obecnie napisania nowej klasy pochodnej po `base_class` z nadpisanymi metodami `Attack()`/`TakeDamage()`. To wciąż jest łatwe i szybkie dzięki polimorfizmowi, ale nie jest to podejście w pełni data-driven, nowa jednostka to wciąż nowy kod C#, a nie np. nowy `ScriptableObject` z danymi. Rozważenie przejścia na kompozycję (np. `ScriptableObject` z definicją statystyk i strategii ataku jako komponentu, zamiast dziedziczenia) pozwoliłoby dodawać jednostki bez pisania nowego kodu, co byłoby mocniejszym argumentem w portfolio.
- Import `Unity.Mathematics` w klasie bazowej jest nieużywany, warto usunąć martwy kod.w logice zwycięstwa).
